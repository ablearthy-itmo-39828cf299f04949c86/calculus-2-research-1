...
// currentprojection = orthographic(5, 3, 4, target=H/2);
...
draw(ODPlusDx -- DPlusDx);
label("$D'$", DPlusDx, +Z+1.2*Y);
label("$O''$", ODPlusDx, 0.5*Z-0.75*Y);
dot(DPlusDx);
dot(ODPlusDx);

draw(OD -- ODPlusDx, L=Label("$\Delta x$", position=MidPoint), align=+Y, Bars3());
draw(O -- OD, L=Label("$x$", position=MidPoint), align=+Y, Arrows3(), Bars3());
